...
public class TimerContinuous : MonoBehaviour
{
    private AudioManager audioManager;
    public string achievementName;
    public string achievementDesc;
    public int requiredPlayTimeInSeconds = 3600; // 1 hour in seconds

    private float playTime = 0f; // Track the playtime in seconds
    ...
    private void Start()
    {
        ...
        // Load playtime from PlayerPrefs
    ...
    {
        // Display progress based on playtime
        float fillAmount = playTime / requiredPlayTimeInSeconds;
        if (fillAmount < 0.1f)
        {
            print("Empty");
        }
        ...
        if (playTime >= requiredPlayTimeInSeconds && !isCompleted)
        {
            isCompleted = true;
            COMPLETED();
